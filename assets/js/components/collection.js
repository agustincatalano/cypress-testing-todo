...
Vue.component('board-collection', {
  ...
  sockets: {
    ...
    boardDeleted (id) {
      const deleted = this.boards.findIndex(board => board.id === id)
      this.boards.splice(deleted, 1)
    },
    boardUpdate (id, message) {
      const updated = this.boards.findIndex(board => board.id === id)
      const board = this.boards[updated]
